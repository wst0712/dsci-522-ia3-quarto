...
The analysis was conducted using Python [@Python] along with several Python 
packages: pandas for data manipulation [@pandas], Altair for visualization [@altair], 
and Click for command-line interfaces [@click]. 
This report was rendered with Quarto [@Allaire_Quarto_2022]. 
*Note: This report is adapted from Timbers’ earlier work [@ttimbers-horses].*
...
of the number of horses during the same time span as @fig-horse-pop.

From @tbl-horse-sd, we see that the province with the largest standard deviation 
is `{python} largest_sd`.
...
In @fig-horse-largest we zoom in and look at the province of `{python} largest_sd`, 
